...
from app.schemas.produto import ProdutoCreate, ProdutoResponsse
from app.crud import produto as crud

router = APIRouter(prefix="/produtos", tags=["Produtos"])

@router.get("/", response_model=list[ProdutoResponsse])
def listar_produtos(db: Session = Depends(get_db)):
    return crud.get_produtos(db)


@router.get("/{produto_id}", response_model=ProdutoResponsse)
...
@router.post("/", response_model=ProdutoResponsse)
